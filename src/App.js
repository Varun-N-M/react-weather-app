...
const App = () => {
  ...
  function displayWeather(data) {
    ...
    fiveForecastDate.forEach((weatherItem, index) => {
      if (index !== 0) {
        ...
        const { feels_like, temp_min, temp_max, pressure, humidity } = weatherItem.main
        const { visibility } = weatherItem
        const { speed } = weatherItem.wind

        setCurrentWeather({ name, temp, icon, description, date })
        setMoreDetails({ feels_like, temp_min, temp_max, pressure, humidity, sunrise, sunset, country, speed, visibility })
      }
    });
  ...
  return (
    <div >
      <Navbar onCityChange={handleCityChange} getUserCoordinates={getUserCoordinates} />
      <main>
        <div className='sidebar'>
          {currentWeather && <aside><Currentweather city={city} currentWeather={currentWeather} /></aside>}
          {forecast.length > 0 && <aside className='forecast-data'>{forecast.map((item, index) => (<Forecast key={index} forecast={item} />))}</aside>}
        </div>
